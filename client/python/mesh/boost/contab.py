...
class CronTab:
    ...
    def __init__(self, user=None, tab=None, tabfile=None):
        self.lines = None
        self.crons = None
        self.filen = None
    ...
    def _set_parse(self, result, line=""):
        ...
        if self.cron.user is False:
            ...
            if len(ret) == 2:
                ...
                self.valid = False
                self.enabled = False
                log.warn(str("Missing user or command in system cron %s: %s"),
                         '' if self.cron is None else (self.cron.tabfile or ''), line)
        else:
