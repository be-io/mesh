...
import io.be.mesh.prsim.*;
import io.be.mesh.struct.*;
import io.be.mesh.tool.LDCable;
import io.be.mesh.tool.Mode;
...
/**
 * Like: create.tenant.omega.json.http2.lx000001.mpi.ducesoft.com
 *
...
public class MeshEden implements Eden, RuntimeHook, Subscriber {
    ...
    private Environ getEnviron() {
        Network network = io.be.mesh.mpc.ServiceLoader.load(Network.class).getDefault();
    ...
    public void stop() throws Throwable {
        Scheduler scheduler = io.be.mesh.mpc.ServiceLoader.load(Scheduler.class).getDefault();
        scheduler.cancel(aliveId);
        scheduler.shutdown(Duration.ofSeconds(3));
        Registry registry = io.be.mesh.mpc.ServiceLoader.load(Registry.class).getDefault();
    ...
    public void subscribe(Event event) {
        Builtin builtin = io.be.mesh.mpc.ServiceLoader.load(Builtin.class).getDefault();
    ...
    private static final class Instance<T> implements Execution<T> {
        ...
        private Instance(String urn, Class<?> type, Method method, Object target, T resource) {
            ...
            this.intype = io.be.mesh.mpc.ServiceLoader.load(JCompiler.class).get(JCompiler.JAVASSIST).intype(this.method);
