...
import io.be.mesh.mpc.*;
import com.jiumi.mesh.mpc.*;
import io.be.mesh.mpc.ServiceLoader;
import io.be.mesh.prsim.Cache;
import io.be.mesh.prsim.Context;
import io.be.mesh.struct.CacheEntity;
...
@Slf4j
@Setter
@SPI(value = "javax-boost", priority = Integer.MIN_VALUE)
public class ServletBoostFilter implements Filter {
    ...
    private String removePathx = "";

    private String getRemovePath() {
        return Tool.anyone(removePathx, Tool.getProperty("", "MESH_SESSION_REMOVE_PATH", "mesh.session.remove.path"));
    }
            ...
            for (Cookie cookie : request.getCookies()) {
                if (Tool.equals(SESSION_ID, cookie.getName())) {
                    if (cookie.getMaxAge() < 1) {
                        cookie.setDomain(String.format(".%s", request.getServerName()));
                        cookie.setPath("/");
