...
var (
	consumers = new(macro.Once[map[string]Consumer]).With(func() map[string]Consumer {
		cs := map[string]Consumer{}
		for _, spi := range macro.Load(IConsumer).List() {
			if c, ok := spi.(Consumer); ok {
				cs[spi.Att().Name] = c
			}
		}
		return cs
	})
	codecs = new(macro.Once[map[string]codec.Codec]).With(func() map[string]codec.Codec {
		cs := map[string]codec.Codec{}
		for _, spi := range macro.Load(codec.ICodec).List() {
			if c, ok := spi.(codec.Codec); ok {
				cs[spi.Att().Name] = c
			}
		}
		return cs
...
func (that *ReferenceHandler) ReferExecution(ctx context.Context, inspector macro.Inspector) (Execution, error) {
	execution, err := aware.Eden.Refer(ctx, that.MPI, inspector.GetDeclaredKind(), inspector)
	if nil != err {
...
func (that *ReferenceHandler) withConsumer(name string) (Consumer, error) {
	if cc := consumers.Get()[name]; nil != cc {
		return cc, nil
	}
	return nil, cause.Errorf("No consumer named %s ", name)
}

func (that *ReferenceHandler) withCodec(name string) (codec.Codec, error) {
	if cc := codecs.Get()[name]; nil != cc {
		return cc, nil
	}
	return nil, cause.Errorf("No codec named %s ", name)
}
...
func (that *ReferenceHandler) Invoke(ctx context.Context, invocation Invocation) (interface{}, error) {
	...
	cname := MeshFlag.OfProto(invocation.GetURN().Flag.Proto).Name()
	consume, err := that.withConsumer(cname)
	if nil != err {
		return nil, cause.Error(err)
	}
	cdc, err := that.withCodec(execution.Schema().GetCodec())
	if nil != err {
		return nil, cause.Error(err)
	}
