...
func (that *messages) match(ctx context.Context, kind reflect.Type) (codec, error) {
	if !implements(kind, messageType) {
		return nil, nil
	}
	return &pb{kind: kind}, nil
}
...
func (that *pb) wire() wireType {
	return varlen
}

func (that *pb) size(ctx context.Context, pointer unsafe.Pointer, flags flags) int {
	if pointer != nil {
...
func (that *pb) encode(ctx context.Context, buff []byte, pointer unsafe.Pointer, flags flags) (int, error) {
	if pointer != nil {
		if m := reflect.NewAt(that.kind, pointer).Interface().(Message); m != nil {
			size := m.Size()
			...
			if flags.has(toplevel) {
				...
				return len(buff), m.Marshal(buff)
			}
			...
			n, err := encodeVarint(buff, uint64(size))
			if nil != err {
				return n, err
			}

			return vlen, m.Marshal(buff[n:])
		}
	}
	return 0, nil
}

func (that *pb) decode(ctx context.Context, buff []byte, pointer unsafe.Pointer, flags flags) (int, error) {
