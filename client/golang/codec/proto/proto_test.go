...
func TestMarshalUnmarshal(t *testing.T) {
	...
	values := []interface{}{
		...
		// pointers
		struct{ M *testMessage }{M: nil},
		struct {
			M1 *testMessage
			M2 *testMessage
			M3 *testMessage
		}{
			M1: &testMessage{A: 10, B: 100, C: 1000},
			M2: &testMessage{S: testSubmessage{X: "42"}},
		},
		...
		struct {
			...
			M3 map[string]testMessage
			M4 map[string]*testMessage
			M5 map[key]uint
		}{
			M1: map[int]int{0: 1},
			...
			M3: map[string]testMessage{
				"m0": testMessage{},
				"m1": testMessage{A: 42},
				"m3": testMessage{S: testSubmessage{X: "X", Y: "Y"}},
			},
			M4: map[string]*testMessage{
				"m0": &testMessage{},
				"m1": &testMessage{A: 42},
	...
	for _, v := range values {
		t.Run(fmt.Sprintf("%T/%+v", v, v), func(t *testing.T) {
			...
			if err := Unmarshal(ctx, b, p.Interface()); nil != err {
				t.Fatal(err)
			}
