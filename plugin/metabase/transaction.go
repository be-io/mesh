...
func TXR[T any](ctx context.Context, fn func(ctx context.Context, tx *badger.Txn, ss *bh.Store) (T, error)) (r T, err error) {
	if nil == metabase.Store {
		return r, cause.Errorable(cause.StartPending)
	}
	err = metabase.Store.Badger().Update(func(txn *badger.Txn) error {
...
func TX(ctx context.Context, fn func(ctx context.Context, tx *badger.Txn, ss *bh.Store) error) (err error) {
	if nil == metabase.Store {
		return cause.Errorable(cause.StartPending)
	}
	return metabase.Store.Badger().Update(func(txn *badger.Txn) error {
