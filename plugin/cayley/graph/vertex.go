...
func (that *Graph) AddVerticies(verticies ...Vertex) {
	for _, v := range verticies {
		v.bestVerticies = []int{-1}
		if v.ID >= len(that.Vertexes) {
			newV := make([]Vertex, v.ID+1-len(that.Vertexes))
			that.Vertexes = append(that.Vertexes, newV...)
		}
		that.Vertexes[v.ID] = v
	}
}
