...
func Import(filename string) (g Graph, err error) {
	...
	for _, line := range strings.Split(input, "\n") {
		...
		if temp := len(g.Vertexes); temp <= i { //Extend if we have to
			g.Vertexes = append(g.Vertexes, make([]Vertex, 1+i-len(g.Vertexes))...)
			for ; temp < len(g.Vertexes); temp++ {
				g.Vertexes[temp].ID = temp
				g.Vertexes[temp].arcs = map[int]int64{}
				g.Vertexes[temp].bestVerticies = []int{-1}
			}
		}
		if len(f) == 1 {
...
func (that Graph) ExportToFile(filename string) error {
	var i string
	var err error
	...
	for _, v := range that.Vertexes {
		if that.usingMap {
			if i, err = that.GetMapped(v.ID); err != nil {
		...
		for key, val := range v.arcs {
			if that.usingMap {
				if i, err = that.GetMapped(key); err != nil {
					return errors.New("mapping fail when exporting; " + err.Error())
				}
				fmt.Fprint(f, " ", i, ",", val)
			} else {
