...
func testExport(t *testing.T, g Graph, graph string) {
	...
	got, _ := Import(f)
	if len(g.Vertexes) != len(got.Vertexes) {
		t.Fatal("Vertexes not same size", g.Vertexes, got.Vertexes, "\n", graph)
	}
	for i := range g.Vertexes {
		if !reflect.DeepEqual(g.Vertexes[i], got.Vertexes[i]) {
			t.Error("Vertex does not match", g.Vertexes[i], got.Vertexes[i], "\n", graph)
		}
	}
...
func TestImportCorrectMap(t *testing.T) {
	...
	f := "testdata/L.txt"
	test(t, Graph{
		Vertexes: []Vertex{{ID: 0}, {ID: 1}, {ID: 2}}},
		map[string]int{
			"A": 0, "B": 1, "C": 2,
		}, nil, f)
...
func assertGraphsEqual(t *testing.T, a, b Graph) {
	if len(a.Vertexes) != len(b.Vertexes) {
		t.Fatal("Error in graph sizes a size:", len(a.Vertexes), "\tb size:", len(b.Vertexes))
	}
}
