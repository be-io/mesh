...
import (
	...
	"github.com/be-io/mesh/plugin/proxy"
	"os"
	"sort"
	"strings"
	"time"
...
func (that *PRSINetwork) Index(ctx context.Context, index *types.Paging) (*types.Page, error) {
...
func (that *PRSINetwork) Version(ctx context.Context, nodeId string) (*types.Versions, error) {
	...
	return proxy.InspectVersion(mpc.ContextWith(ctx), nodeId)
}

func (that *PRSINetwork) Instx(ctx context.Context, index *types.Paging) (*types.Page, error) {
