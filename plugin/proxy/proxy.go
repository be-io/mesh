...
const (
	...
	PluginHeader      = "header"
	PluginForwarder   = "forwarder"
	PluginAuthority   = "authority"
	PluginTransformer = "httpformer"
	PluginErrors      = "errors"
	PluginRewrite     = "rewrite"
	PluginHealth      = "health"
	PluginSubfilter   = "subfilter"
	PluginINC         = "inc"
	PluginWedge       = "wedge"
...
func (that *meshProxy) Start(ctx context.Context, runtime plugin.Runtime) {
	...
	if nil != err {
		log.Error(ctx, "Could not set round robin default weight: %v", err)
		return
	}
	config.SetEffectiveConfiguration()
	if err = config.ValidateConfiguration(); nil != err {
...
func (that *meshProxy) setupServer(ctx context.Context, staticConfiguration *static.Configuration) (*server.Server, error) {
	...
	if staticConfiguration.Hub != nil {
		if err = providerAggregator.AddProvider(staticConfiguration.Hub); nil != err {
			return nil, cause.Errorf("adding Hub provider: %w", err)
		}

		// API is mandatory for Hub to access the dynamic configuration.
	...
	watcher.AddListener(func(config dynamic.Configuration) {
		for rtName, rt := range config.HTTP.Routers {
			...
			if _, ok := resolverNames[rt.TLS.CertResolver]; !ok && (staticConfiguration.Hub == nil || rt.TLS.CertResolver != "traefik-hub") {
...
func (that *meshProxy) getDefaultsEntrypoints(ctx context.Context, staticConfiguration *static.Configuration) []string {
	...
	for name, cfg := range staticConfiguration.EntryPoints {
		...
		// Hub entryPoint should not be used as a default entryPoint.
...
func (that *meshProxy) registerMetricClients(ctx context.Context, metricsConfig *types.Metrics) []metrics.Registry {
	...
	if metricsConfig.InfluxDB != nil {
		...
		log.Debug(ctx, "Configured InfluxDB metrics: pushing to %s once every %s",
