...
func ListenUDP(network string, laddr *net.UDPAddr) (*net.UDPConn, error) {
	...
	if err = syscall.SetsockoptInt(fileDescriptor, syscall.SOL_IP, syscall.IP_RECVORIGDSTADDR, 1); nil != err {
...
func ReadFromUDP(conn *net.UDPConn, b []byte) (int, *net.UDPAddr, *net.UDPAddr, error) {
	...
	for _, msg := range msgs {
		if msg.Header.Level == syscall.SOL_IP && msg.Header.Type == syscall.IP_RECVORIGDSTADDR {
			...
			if err = binary.Read(bytes.NewReader(msg.Data), binary.LittleEndian, originalDstRaw); nil != err {
...
func DialUDP(network string, laddr *net.UDPAddr, raddr *net.UDPAddr) (*net.UDPConn, error) {
	...
	if err = syscall.Connect(fileDescriptor, remoteSocketAddress); nil != err {
		syscall.Close(fileDescriptor)
