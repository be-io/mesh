...
func (that *circulate) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	mdc := prsim.MeshMDC.GetHeader(request.Header)
	// Max circulate times 5
	if strings.Count(mdc, circulates.MDC) < 6 {
		if strings.Count(mdc, ":") < 31 {
			prsim.MeshMDC.SetHeader(request.Header, fmt.Sprintf("%s:%s", mdc, circulates.MDC))
		}
		that.next.ServeHTTP(writer, request)
		return
	}
