...
func (that *SnapShot) withRemoteRoute(ctx context.Context, routes *Routes) {
	for _, route := range that.routers {
		...
		rr := fmt.Sprintf("(HeaderRegexp(`x-ptp-target-node-id`, `%s`) && (PathRegexp(`/v1/interconn/chan/invoke.*`) || PathRegexp(`/v1/interconn/chan/transport.*`) || PathRegexp(`/org.ppc.ptp.PrivateTransferProtocol/invoke.*`) || PathRegexp(`/org.ppc.ptp.PrivateTransferProtocol/transport.*`))) || HeaderRegexp(`mesh-urn`, `%s`)", route.NodeId, mtypes.URNMatcher("([-a-zA-Z\\d]+)", route.ID(ctx).SEQ, mtypes.CN, route.NodeId, route.InstId))
