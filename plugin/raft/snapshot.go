...
func init() {
	var _ sm.IOnDiskStateMachine = new(DiskKV)
}
...
// RStore creates a new disk kv test state machine.
func RStore(clusterID uint64, nodeID uint64) sm.IOnDiskStateMachine {
	d := &DiskKV{
		clusterID: clusterID,
		nodeID:    nodeID,
	}
