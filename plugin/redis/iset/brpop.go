...
func (that *Brpop) Length() int {
	return 3
}

func (that *Brpop) Name() string {
	return "BRPOP"
}
...
func (that *Brpop) Serve(ctx context.Context, conn Conn, cmd redcon.Command, client redis.Cmdable) {
	...
	for index, key := range cmd.Args {
		...
		r, err := client.BRPop(ctx, time.Second*time.Duration(expire), string(key)).Result()
		if IsNil(err) {
			continue
		}
		if nil != err {
